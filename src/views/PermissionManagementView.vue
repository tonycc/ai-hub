...
const riskLevelMap = Object.fromEntries(riskLevelOptions)
...
const dataScopeMap = Object.fromEntries(dataScopeOptions)
...
<template>
  <div class="page-shell">
    <PageHeader title="权限与安全" description="应用权限点、应用角色和数据范围彼此隔离；对象级最终校验仍由独立应用执行。">
      ...
      <el-select v-model="applicationId" filterable placeholder="选择应用" style="width: 260px"><el-option v-for="item in applications" :key="item.application_id" :label="item.name" :value="item.application_id" /></el-select>
    ...
    <section class="surface-panel page-section list-panel"><ApiState :loading="loading" :error="error" :empty="!applicationId" empty-text="请选择一个应用" @retry="loadResources">
      ...
      <el-table v-else :data="assignments" style="width: 100%"><el-table-column prop="display_name" label="用户" min-width="180" /><el-table-column prop="subject" label="身份主体" min-width="210"><template #default="scope"><code>{{ scope.row.subject }}</code></template></el-table-column><el-table-column prop="role_name" label="应用角色" min-width="180" /><el-table-column prop="data_scope_type" label="数据范围" width="160"><template #default="scope">{{ dataScopeMap[scope.row.data_scope_type] || scope.row.data_scope_type }}</template></el-table-column><el-table-column label="范围参数" min-width="260"><template #default="scope"><code>{{ JSON.stringify(scope.row.data_scope) }}</code></template></el-table-column></el-table>
    </ApiState></section>

    <el-drawer v-model="permissionVisible" title="登记应用权限点" size="min(580px, 96vw)"><el-form label-position="top"><el-form-item label="权限编码" required><el-input v-model="permissionForm.permission_code" placeholder="example.record.read" /></el-form-item><el-form-item label="名称" required><el-input v-model="permissionForm.name" /></el-form-item><el-form-item label="说明" required><el-input v-model="permissionForm.description" type="textarea" :rows="3" /></el-form-item><el-form-item label="风险等级"><el-select v-model="permissionForm.risk_level" style="width: 100%"><el-option v-for="item in riskLevelOptions" :key="item[0]" :label="item[1]" :value="item[0]" /></el-select></el-form-item></el-form><template #footer><el-button @click="permissionVisible = false">取消</el-button><el-button type="primary" :loading="saving" @click="savePermission">登记</el-button></template></el-drawer>
    <el-drawer v-model="roleVisible" title="创建应用角色" size="min(600px, 96vw)"><el-form label-position="top"><el-form-item label="角色名称" required><el-input v-model="roleForm.name" /></el-form-item><el-form-item label="说明" required><el-input v-model="roleForm.description" type="textarea" :rows="3" /></el-form-item><el-form-item label="权限点" required><el-select v-model="roleForm.permission_codes" multiple style="width: 100%"><el-option v-for="item in permissions.filter((value) => value.status === 'ACTIVE')" :key="item.permission_code" :label="`${item.name} · ${item.permission_code}`" :value="item.permission_code" /></el-select></el-form-item></el-form><template #footer><el-button @click="roleVisible = false">取消</el-button><el-button type="primary" :loading="saving" @click="saveRole">创建</el-button></template></el-drawer>
    <el-drawer v-model="assignmentVisible" title="分配应用授权" size="min(600px, 96vw)"><el-form label-position="top"><el-form-item label="用户" required><el-select v-model="assignmentForm.user_id" filterable style="width: 100%"><el-option v-for="item in users" :key="item.user_id" :label="`${item.display_name} · ${item.subject}`" :value="item.user_id" /></el-select></el-form-item><el-form-item label="应用角色" required><el-select v-model="assignmentForm.role_id" style="width: 100%"><el-option v-for="item in roles.filter((value) => value.status === 'ACTIVE')" :key="item.role_id" :label="item.name" :value="item.role_id" /></el-select></el-form-item><el-form-item label="数据范围"><el-select v-model="assignmentForm.data_scope_type" style="width: 100%"><el-option v-for="item in dataScopeOptions" :key="item[0]" :label="item[1]" :value="item[0]" /></el-select></el-form-item><el-form-item v-if="assignmentForm.data_scope_type !== 'GLOBAL'" label="数据范围参数（JSON）" required><el-input v-model="assignmentForm.data_scope_text" type="textarea" :rows="3" /></el-form-item></el-form><template #footer><el-button @click="assignmentVisible = false">取消</el-button><el-button type="primary" :loading="saving" @click="saveAssignment">分配</el-button></template></el-drawer>
  </div>
</template>
