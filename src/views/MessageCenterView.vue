...
<template>
  <div class="page-shell">
    <PageHeader title="通知中心" description="配置站内测试通知并验证幂等、状态和审计链路；当前不包含邮件、短信或协作软件等外部通道。">
      ...
      <el-select v-if="activeTab === 'messages'" v-model="statusFilter" clearable placeholder="全部状态" style="width: 145px" @change="loadAll"><el-option v-for="item in [['PENDING','待处理'],['DELIVERED','已送达'],['FAILED','失败']]" :key="item[0]" :label="item[1]" :value="item[0]" /></el-select>
    ...
    <el-drawer v-model="configVisible" title="配置站内测试通知" size="min(520px, 96vw)"><el-form label-position="top"><el-form-item label="应用" required><el-select v-model="configForm.application_id" style="width: 100%"><el-option v-for="item in applications.filter((app) => session.hasPermission('platform.notification.write', app.application_id))" :key="item.application_id" :label="item.name" :value="item.application_id" /></el-select></el-form-item><el-form-item label="发送方名称" required><el-input v-model="configForm.sender_name" /></el-form-item><el-form-item label="启用站内测试通道"><el-switch v-model="configForm.enabled" /></el-form-item></el-form><template #footer><el-button @click="configVisible = false">取消</el-button><el-button type="primary" :loading="saving" @click="saveConfig">保存</el-button></template></el-drawer>
    <el-drawer v-model="testVisible" title="发送站内测试通知" size="min(560px, 96vw)"><el-form label-position="top"><el-form-item label="应用" required><el-select v-model="testForm.application_id" style="width: 100%" @change="changeTestApplication"><el-option v-for="item in applications.filter((app) => session.hasPermission('platform.notification.write', app.application_id))" :key="item.application_id" :label="item.name" :value="item.application_id" /></el-select></el-form-item><el-form-item label="站内收件人" required><el-select v-model="testForm.recipient_user_id" filterable style="width: 100%"><el-option v-for="item in users" :key="item.user_id" :label="`${item.display_name} · ${item.subject}`" :value="item.user_id" /></el-select></el-form-item><el-form-item label="主题" required><el-input v-model="testForm.subject" /></el-form-item><el-form-item label="正文" required><el-input v-model="testForm.body" type="textarea" :rows="4" /></el-form-item><el-form-item label="幂等键"><el-input v-model="testForm.idempotency_key" disabled /></el-form-item></el-form><template #footer><el-button @click="testVisible = false">取消</el-button><el-button type="primary" :loading="saving" :disabled="!testForm.recipient_user_id" @click="sendTest">发送站内测试</el-button></template></el-drawer>
  </div>
</template>

<style scoped>.management-tabs{display:flex;gap:24px}.management-tabs button{position:relative;height:46px;padding:0;border:0;color:var(--ink-500);background:transparent;font-size:15px;cursor:pointer}.management-tabs button.active{color:var(--ink-900);font-weight:650}.management-tabs button.active::after{position:absolute;right:0;bottom:0;left:0;height:2px;background:var(--accent-500);content:''}.management-tabs em{margin-left:5px;color:#89959d;font-size:var(--font-eyebrow);font-style:normal}.channel-scope{display:grid;grid-template-columns:46px minmax(0,1fr) auto;align-items:center;gap:var(--space-gap);padding:var(--space-card-lg)}.channel-scope>span{display:grid;width:44px;height:44px;border-radius:8px;color:#416f86;background:#edf4f7;font-size:20px;place-items:center}.channel-scope>div{display:grid;gap:4px}.channel-scope strong{color:var(--ink-900);font-size:var(--font-body-lg)}.channel-scope small{max-width:760px;color:var(--ink-500);font-size:var(--font-caption);line-height:1.6}.channel-scope dl{display:flex;gap:24px;margin:0}.channel-scope dl div{display:grid;gap:4px}.channel-scope dt{color:var(--ink-500);font-size:var(--font-caption)}.channel-scope dd{margin:0;color:var(--ink-700);font-size:var(--font-body)}.list-panel{min-height:460px;overflow:hidden}.subline{display:block;margin-top:4px;color:var(--ink-500);font-size:var(--font-caption)}code{color:#416c83}@media(max-width:850px){.channel-scope{grid-template-columns:46px 1fr}.channel-scope dl{grid-column:2}}@media(max-width:560px){.channel-scope dl{display:grid;gap:var(--space-gap)}}</style>
