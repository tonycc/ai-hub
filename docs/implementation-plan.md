...
M0、M1、M2 和 M3 已完成。下一步从 M4-01 冻结服务窗口、SLO、RPO、RTO 和数据保留目标；API-only 应用继续只运行 `base-access`，不会因可靠事件能力被强制安装 Outbox、Inbox 或事件 Worker。后续变更通过 Pull Request 合入受保护的 `main`。
