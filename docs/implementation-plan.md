...
M0、M1、M2、M3 和 M4-01 至 M4-04 已完成。下一步执行 M4-05 性能、安全、慢调用、事件积压、重试风暴和公共依赖故障演练；API-only 应用继续只运行 `base-access`，不会因可靠事件能力被强制安装 Outbox、Inbox 或事件 Worker。后续变更通过 Pull Request 合入受保护的 `main`。
