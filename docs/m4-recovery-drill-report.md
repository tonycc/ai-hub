...
| 完整服务 RTO | ≤ 120 分钟 | 454 秒（7 分 34 秒） | 通过 |
| 工具恢复耗时 | 记录项 | 3.142 秒 | 通过 |
...
演练时间：2026-08-13（Asia/Shanghai）。环境：本机 Docker Compose 隔离项目，`standard-events` profile，归档存储类别 `local-drill`。生产 RPO 仍必须由小时定时器和异机存储成功时间持续证明，本次本机演练不冒充生产异机备份。
...
3. 生成 AES-256-GCM 加密归档，独立验证 sidecar、认证标签、内部摘要和清单。
...
{
  "passed": true,
  "backup_id": "ai-hub-backup-20260813T144540Z-8f088b10",
  ...
  "tool_restore_seconds": 3.142,
  "total_recovery_seconds": 454,
...
无未关闭的恢复整改项。M4 最终验收还需继续完成指标告警、发布回滚、凭据轮换、性能与故障演练。
