...
发布管理操作通过应用行和环境行数据库锁串行化，避免多实例同时轮换产生多个 `ACTIVE` 凭据。存在多个可选版本时，未指定凭据 ID 的旧式吊销请求会失败关闭。当前没有未关闭的 M4-04 整改项；后续性能、安全和综合故障演练已完成，汇总结论见 [M4 最终验收报告](m4-final-acceptance-report.md)。
