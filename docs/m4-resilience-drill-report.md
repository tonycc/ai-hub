...
2026-08-14 在全新隔离的 `base-access` Docker Compose 环境执行 `scripts/ci/m4-resilience-runtime.sh`，M4-05 的性能、安全、慢调用、数据汇聚滞后、有限重试和公共依赖故障演练全部通过。
...
| 数据同步滞后恢复 | 依赖恢复后拉取续跑 | 恢复后从位点继续增量拉取 | 通过 |
| 数据库故障重试 | 有限重试，不形成无限风暴 | ingest 调度器有限重试并告警 | 通过 |
...
运行门禁从 `deploy/operations/production-targets.json` 读取性能、错误率和恢复目标；平台启动时也加载并校验同一个文件。目标文件缺失、格式无效、档位不一致或阈值自相矛盾时，服务失败关闭。运维摘要告警状态与演练断言使用同一来源。
...
- 数据库运行角色继续满足平台核心、raw 贴源层、独立应用和迁移边界，不能跨边界读取或写入。
...
演练暂停 ingest 调度并积累数据变化。恢复调度后平台从位点继续增量拉取，当前态与变更记录保持一致；对账无漂移。停止 PostgreSQL 期间调度器执行有限重试并告警，恢复后 API、独立应用和调度器均恢复健康，没有出现无限重试或静默丢失。

## 6. 演练发现与关闭项

演练迭代中发现并关闭两项问题：

1. 同步滞后告警阈值曾在服务中硬编码，现统一从生产目标文件加载并在启动时校验。
