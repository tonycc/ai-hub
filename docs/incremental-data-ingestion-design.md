...
6. **应用主动推送（已被 ADR-033 替代）**：原冻结为「推送只作加速信号，不承载业务数据，首期不做」。该条已由 [ADR-033](adr/ADR-033-push-agent-data-ingest-transport.md) 替代：`PUSH_AGENT` 可作为现有 `DATA_INGEST` 的数据面传输，权威对象记录进入现有 `platform_raw`。Webhook 式「仅提示再拉取」不再作为本期方案。生产 Push 默认关闭，须通过立项 A 的 C1 门禁后按来源启用。细节见 [data2agent 核心能力复制升级实施方案](data2agent-core-copy-upgrade-implementation-plan.md)。
