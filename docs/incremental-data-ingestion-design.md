...
1. **同步周期默认值**：建议按对象类型配置（如客户 1h、设备 15min），需结合业务对时效的真实要求定。
2. **历史保留策略**：方向上建议**"按对象保留最近 N 版本 + 定期快照压实"**（`raw_change_record` 只增不减会无界增长，需先于其他问题到来前定方向），具体窗口再结合 M4 的 60 分钟 RPO / 数据保留基线冻结。
3. **大对象/大批量**：单批次记录数上限与分页策略；超大 payload 是否需要对象存储（暂不引入，按需评估）。
4. **全量重建的触发方式**：手动运维命令 vs 定期自动，需结合对账结果定。
