...
当前不升级高可用。出现以下任一已批准条件时重新评审：服务目标提高到 99.9% 或 7×24、RPO ≤ 5 分钟、RTO ≤ 30 分钟、持续公共 API 负载超过 100 RPS、单主机维护不可接受、法规要求物理故障域隔离。升级顺序是外部托管 PostgreSQL/备份、无状态组件双实例、RabbitMQ 多节点，仍不自动拆分业务模块。
