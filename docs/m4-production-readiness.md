...
M4 采用 `STANDARD_SINGLE_NODE`：单台受管 Linux 主机运行 Docker Compose，一个 PostgreSQL 实例承载隔离逻辑数据库。外部应用继续独立部署，平台不读取其业务数据库。
...
| 可重建汇聚数据 RTO | ≤ 240 分钟；汇聚数据不作为业务事实备份来源 |

可用性范围包含 Traefik、平台 API/门户、authentik 和必要 PostgreSQL。某个独立应用不可用不计为平台 API 不可用，但必须产生应用责任路由告警。
...
- 审计保留 365 天，通知请求保留 90 天，过期门户会话和接入证据再宽限 7 天。汇聚变更记录与当前态按保留策略裁剪，历史版本到期清理。
...
M4-06 根据全部演练结果决定当前不升级高可用，正式记录见 [ADR-031](adr/ADR-031-standard-single-node-production-tier.md)。出现以下任一已批准条件时重新评审：服务目标提高到 99.9% 或 7×24、RPO ≤ 5 分钟、RTO ≤ 30 分钟、持续公共 API 负载超过 100 RPS、单主机维护不可接受、法规要求物理故障域隔离。升级顺序是外部托管 PostgreSQL/备份、无状态组件双实例，仍不自动拆分业务模块。
