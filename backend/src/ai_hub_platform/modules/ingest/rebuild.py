...
async def _matching_ingest_source(
    ...
    *,
    ...
    async with sessions() as session:
        configs = await load_source_configs_from_db(session)
    matching = [
        config
        for config in configs
        if config.source_application_id == source_application_id
        and config.object_type == object_type
    ]
    if not matching:
        raise ValueError(
...
async def sync_configured_source(
    ...
    *,
    ...
    try:
        source = await _matching_ingest_source(
            sessions,
