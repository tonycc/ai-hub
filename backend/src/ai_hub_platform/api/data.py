"""Public platform API for aggregated data current-state and history reads."""
...
router = APIRouter(prefix="/platform-api/v1/data", tags=["data"])

DATA_READ_SCOPE = "platform.data.read"
...
def _map_validation(error: DataQueryValidationError) -> ApiError:
    message = str(error)
    if "another application's" in message or "outside the caller's data scope" in message:
...
async def list_aggregated_objects(
    ...
    principal: Annotated[
        Principal,
        Depends(principal_dependency(DATA_READ_SCOPE, actor_types=("service",))),
    ],
    ...
    try:
        allowed = resolve_service_data_scope(
            caller_application_id=principal.application_id,
            requested_source_application_id=source_application_id,
